...
# Campo de transporte destacado abaixo das colunas
...
if st.button("Gerar Diagnóstico"):
    if not nome_paciente or not genero or not idade or not altura or not peso or not familia or not transporte:
        ...
        st.info("💡 Dica para captura de tela: Utilize 'Windows + Shift + S' (no Windows) ou 'Command + Shift + 4' (no Mac) para salvar a imagem do resultado.")

st.markdown("---")
st.markdown("### 📊 Estatísticas Gerais e Importância das Variáveis do Modelo")

# Exibição do gráfico de importância das variáveis dentro do próprio dashboard
try:
    importances = model.named_steps['classifier'].feature_importances_
    features = model.named_steps['preprocessor'].get_feature_names_out()
    features_limpas = [f.replace("num__", "").replace("cat__", "") for f in features]
    feat_df = pd.DataFrame({'Variável': features_limpas, 'Importância': importances}).sort_values(by='Importância', ascending=False).head(10)
    
    fig, ax = plt.subplots(figsize=(10, 4))
    sns.barplot(x='Importância', y='Variável', data=feat_df, ax=ax, palette='viridis')
    ax.set_title("Top 10 Indicadores Mais Relevantes para a Tomada de Decisão")
    ax.set_xlabel("Grau de Importância Preditiva")
    ax.set_ylabel("Atributo Analisado")
    st.pyplot(fig)
except Exception as e:
    st.caption("Gráficos estatísticos disponíveis após o carregamento completo do arquivo do modelo.")
