...
if st.button("Realizar Previsão"):
    # Tradutores internos de escala para o modelo ler os números do df_modelo
    ...
    # Feature Engineering aplicada no momento da predição
    imc = round(peso / (altura ** 2), 2)
    score_atividade = mapa_ativ_fisica[ativ_fisica] * mapa_telas[telas]

    # Montagem do dicionário sem Peso e sem Altura (evitando o viés)
    dados_entrada = {
        'Genero': genero, 
        'Idade': float(idade), 
    ...
    try:
        ...
        st.error(f"Erro interno ao processar a predição: {e}")
